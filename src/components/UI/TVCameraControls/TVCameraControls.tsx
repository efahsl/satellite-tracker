...
const DirectionalArrow: React.FC<DirectionalArrowProps> = ({
  ...
  return (
    ...
    >
      <span className={styles.arrowSymbol}>{arrowSymbols[direction]}</span>
    </div>
  );
};
...
const ZoomInstructionText: React.FC<ZoomInstructionTextProps> = ({
  ...
  return (
    <div className={styles.zoomText}>
      <span className={textClasses}>{getZoomText()}</span>
    </div>
  );
};
...
export const TVCameraControls: React.FC<TVCameraControlsProps> = ({
  ...
  const shouldShowControls =
    visible && isTVProfile && !uiState.hamburgerMenuVisible && manualMode;

  // Use hook values if props are not provided (hook takes precedence for state management)
  const activeDirection =
    propActiveDirection !== undefined
      ? propActiveDirection
      : hookActiveDirection;
  const pressedDirection =
    propPressedDirection !== undefined
      ? propPressedDirection
      : hookPressedDirection;
  const isInZoomMode =
    propIsInZoomMode !== undefined ? propIsInZoomMode : hookIsInZoomMode;
  const activeZoomDirection =
    propActiveZoomDirection !== undefined
      ? propActiveZoomDirection
      : hookActiveZoomDirection;
      ...
      <div className={styles.arrowsContainer}>
        ...
        <div className={styles.arrowsMiddleRow}>
          <DirectionalArrow
            ...
            isActive={
              !isInZoomMode && activeDirection === TV_CAMERA_DIRECTIONS.LEFT
            }
            isPressed={
              !isInZoomMode && pressedDirection === TV_CAMERA_DIRECTIONS.LEFT
            }
          ...
          <DirectionalArrow
            ...
            isActive={
              !isInZoomMode && activeDirection === TV_CAMERA_DIRECTIONS.RIGHT
            }
            isPressed={
              !isInZoomMode && pressedDirection === TV_CAMERA_DIRECTIONS.RIGHT
            }
