...
import { TextureLoader, Mesh, ShaderMaterial, Vector3, MeshBasicMaterial, MeshPhongMaterial } from 'three';
import { 
  EARTH_RADIUS, 
  ROTATION_SPEED,
...
const Earth: React.FC<EarthProps> = memo(({
  ...
  const actualNormalMap = (normalMapPath && earthQuality !== 'low') ? normalMap : null;
  const actualSpecularMap = (specularMapPath && earthQuality === 'high') ? specularMap : null;

  // References for rotation animation
  const earthRef = useRef<Mesh>(null);
  const materialRef = useRef<ShaderMaterial>(null);

  // Create tier-specific materials
  const materials = useMemo(() => {
    if (earthQuality === 'low') {
      // Low quality: Basic material with day texture only
      return {
        earth: new MeshBasicMaterial({
          map: dayMap,
          transparent: true,
          opacity: 0.9
        })
      };
    } else if (earthQuality === 'medium') {
      // Medium quality: Phong material with basic lighting
      return {
        earth: new MeshPhongMaterial({
          map: dayMap,
          normalMap: actualNormalMap,
          shininess: 30,
          transparent: true,
          opacity: 0.9
        })
      ...
      // High quality: Custom shader with full effects
      return {
        earth: new ShaderMaterial({
          uniforms: {
            ...
            hasSpecularMap: { value: actualSpecularMap ? 1.0 : 0.0 },
  ...
  }, [earthQuality, dayMap, nightMap, actualNormalMap, actualSpecularMap, sunPosition, cityEffects]);
  ...
  useFrame(() => {
    ...
    if (materialRef.current && earthQuality === 'high') {
  ...
  return (
    <group>
      ...
      <mesh ref={earthRef}>
        ...
        <primitive object={materials.earth} ref={materialRef} />
      </mesh>
